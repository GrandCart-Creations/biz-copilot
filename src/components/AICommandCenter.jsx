...
import { authorizeAIScope, AI_SCOPES, getDefaultScopeForRole } from '../utils/accessGateway';
...
const AICommandCenter = () => {
  ...
  const { userRole, currentCompanyId } = useCompany();
  const [isOpen, setIsOpen] = useState(false);
  const [query, setQuery] = useState('');
  const [scope, setScope] = useState(getDefaultScopeForRole(userRole));
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setScope(getDefaultScopeForRole(userRole));
    }
  }, [isOpen, userRole]);
  ...
  const scopeOptions = useMemo(() => {
    const base = [
      ...
      { value: AI_SCOPES.OWNER, label: 'Owner-only insights' }
    ];
    return base;
  }, []);
  ...
  const runQuery = async (event) => {
    ...
    const authorization = authorizeAIScope({ role: userRole, scope, accessCode, query });
      ...
      <div className="relative w-full max-w-2xl bg-white border border-gray-200 rounded-2xl shadow-xl">
        <form onSubmit={runQuery} className="p-6 border-b border-gray-200">
          <div className="flex flex-wrap gap-3 items-center mb-4">
            <span className="px-2 py-1 text-xs font-semibold uppercase tracking-wide text-[#005E7C] bg-[#0DBFA6]/10 rounded">
              AI Command Center
            </span>
          ...
          <div className="space-y-3">
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0DBFA6]/40"
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0DBFA6]/40"
                >
        ...
        <div className="p-6 grid gap-3 sm:grid-cols-3">
          {SUGGESTION_PRESETS.map((preset) => (
            <button
              ...
              className="text-left border border-gray-200 rounded-lg px-3 py-3 hover:border-[#0DBFA6] hover:bg-[#0DBFA6]/5 transition"
            >
